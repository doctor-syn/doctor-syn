...
#[test]
fn test_interpreter() {
    let interp = interpreter::Interpreter::new_x(0.5);

    // Block.
    let e : Block = parse_quote!({x});
    assert_eq!(interp.block(&e).unwrap(), 0.5);

    // Variable.
    let e : Expr = parse_quote!(x);
    assert_eq!(interp.expr(&e).unwrap(), 0.5);
    ...
    assert_eq!(interp.expr(&e).unwrap(), 999999.0);
